      ...
      <div className="flex">
        ...
        <aside className="hidden lg:flex flex-col w-64 bg-sidebar min-h-screen sticky top-0 border-r border-sidebar-border">
          ...
          <nav className="p-4 space-y-1 flex-1">
            {adminNavItems.map(item => <Link key={item.path} to={item.path} className={cn('flex items-center gap-3 px-4 py-3 rounded-lg text-sidebar-foreground transition-all duration-200', location.pathname === item.path ? 'bg-sidebar-accent text-sidebar-primary shadow-sm' : 'hover:bg-sidebar-accent/50 hover:text-sidebar-foreground')}>
                <item.icon className={cn("h-5 w-5", location.pathname === item.path ? "text-sidebar-primary" : "text-sidebar-foreground/40")} />
                <span className="flex-1 font-medium">{item.label}</span>
                {item.badge ? <Badge variant="destructive" className="h-5 min-w-[20px] px-1.5 text-[10px] flex items-center justify-center font-bold">
                    {item.badge}
                  </Badge> : null}
              </Link>)}
          </nav>
          <div className="p-6 border-t border-sidebar-border">
            <Link to="/" className="flex items-center gap-2 text-xs font-medium text-muted-foreground hover:text-primary transition-colors">
